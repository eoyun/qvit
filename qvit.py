...
class SASQuaTChTokenMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        with Timer("SASQ::pre_move", PROFILE_QVIT):
            ...
            target_tokens = n_chunks * self.chunk_tokens
            if target_tokens > self.max_seq_len:
                pad2 = torch.zeros(B, target_tokens - self.max_seq_len, E, dtype=x_cpu.dtype, device=cpu)
                pad2[..., 0] = 1.0
                x_cpu = torch.cat([x_cpu, pad2], dim=1)
